...
def _get_client(api_key: str):
    """Return a Gemini client, or None if no key."""
...
def _token_chunk(text: str, tokenizer, n_tokens: int, n_overlap: int) -> list[str]:
    ...
    while pos < len(ids):
        ...
        chunk = tokenizer.decode(ids[pos:end], skip_special_tokens=True).strip()
        if chunk:
            chunks.append(chunk)
...
def answer_rag(question: str, client) -> tuple[str, list[str]]:
    ...
    _, idxs = st.session_state.faiss_index.search(q_vec.astype(np.float32), TOP_K)
    passages = [
    ...
    try:
        ...
        return r.text.strip(), passages
    except Exception as exc:
        return f"[ERROR] {exc}", passages


def answer_bare(question: str, client) -> str:
    try:
        r = client.models.generate_content(
            model=GEMINI_MODEL,
            contents=question,
...
def _gen_queries(text: str, client, n: int = 8) -> list[str]:
    prompt = (
...
def _llm_judge(query: str, passages: list[str], client) -> bool:
    ctx = "\n\n".join(f"[Chunk {i+1}]: {p[:400]}" for i, p in enumerate(passages))
    prompt = (
        f"Query: {query}\n\nRetrieved context:\n{ctx}\n\n"
...
def _draw_chart(results: dict) -> bytes:
    ...
    for ax, values, ylabel, title, annotation in [
        (ax1, hit_rates, "Hit Rate @5 (%)", "Retrieval Hit Rate by Chunk Size", True),
        (ax2, n_chunks,  "Number of Chunks", "Index Size by Chunk Configuration", False),
    ]:
        ax.set_facecolor(BG)
        ...
        ax.set_title(title, color="#111827", fontsize=11, fontweight="bold", pad=12)
...
with st.sidebar:
    ...
    # ── API Key ──
    st.markdown('<p class="section-title">API Configuration</p>', unsafe_allow_html=True)
    ...
    if not api_key:
        st.warning("Enter a Gemini API key to enable LLM features.", icon="🔑")
    else:
    ...
    new_files = [f for f in (uploaded or []) if f.name not in st.session_state.chunk_counts]
    ...
    if new_files:
        with st.spinner("Indexing…"):
            ...
            if st.session_state.chunks:
                st.session_state.faiss_index = _build_index(st.session_state.chunks)
    ...
    <div style="font-size:12px;color:#4b5563;font-family:'Inter',sans-serif;line-height:2.1;">
        ...
        <div>LLM &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;: <b style="color:#111827;">Gemini 2.5 Flash</b></div>
...
<div style="margin-bottom:24px;">
    ...
    <p style="
        ...
        margin:0;
    ">
        Upload research documents and ask questions. RAG answers are grounded in your documents;
        bare LLM answers come from training data only — compare them to spot hallucination.
    </p>
</div>
""", unsafe_allow_html=True)
...
with tab_query:
    ...
    with btn_col:
        ask = st.button("Ask", type="primary", key="ask_btn", use_container_width=True)
        ...
        if not query.strip():
            ...
            with c1:
                with st.spinner("Retrieving context + generating answer…"):
                    rag_ans, passages = answer_rag(query, client)
                st.session_state.rag_answer   = rag_ans
                st.session_state.ctx_passages = passages
            with c2:
                with st.spinner("Querying bare LLM (no document context)…"):
                    bare_ans = answer_bare(query, client)
                st.session_state.bare_answer = bare_ans
    ...
    if st.session_state.rag_answer is None:
        ...
        ">
            ...
            <div style="font-size:13px;color:#9ca3af;font-family:'Inter',sans-serif;line-height:1.7;">
                ...
                RAG and bare LLM answers will appear side by side.
            </div>
        </div>
        """, unsafe_allow_html=True)
    else:
        ...
        with col_llm:
            ...
            bare_safe = bare_text.replace("&","&amp;").replace("<","&lt;").replace(">","&gt;").replace("\n","<br>")
            st.html(
                '<div style="'
                'background:#ffffff;'
                'border:1px solid #fde68a;'
            ...
            <div style="
                ...
                line-height:1.65;
            ">
                ⚠️ Training data only. Dates, figures, and specific claims may be
                hallucinated or outdated. Cross-reference with the RAG answer above.
            </div>
            """, unsafe_allow_html=True)
...
with tab_eval:
    ...
    <div style="margin-bottom:20px;">
        ...
        <p style="font-size:13px;color:#6b7280;font-family:'Inter',sans-serif;
            ...
            using <strong>LLM-as-judge</strong>. Gemini auto-generates factual test questions from
    ...
    if st.session_state.faiss_index is None:
        st.warning("Upload a document in the sidebar to enable evaluation.", icon="📄")
    elif not api_key:
        st.error("Add your Gemini API key in the sidebar to run evaluation.", icon="🔑")
    else:
        if st.button(
            "Run Evaluation  (auto-generates queries + judges retrieval via Gemini)",
            type="primary",
            key="run_eval",
            ...
            if not combined.strip():
                ...
                try:
                    queries = _gen_queries(combined, client, n=8)
                    prog.progress(15, text=f"Generated {len(queries)} queries. Evaluating chunks…")
                    ...
                    for cfg in eval_cfgs:
                        chunks = _token_chunk(combined, tokenizer, cfg["tokens"], cfg["overlap"])
                        ...
                        for q in queries:
                            ...
                            verdict   = _llm_judge(q, retrieved, client)
                            hits     += int(verdict)
                            detail.append({
                                "Config"  : cfg["label"],
                            ...
                            prog.progress(pct, text=f"{cfg['label']} — {ops_done}/{total_ops}")
...
with tab_about:
    ...
    with c_left:
        ...
        steps = [
            ...
            ("6. RAG Answer",   "Gemini 2.5 Flash answers using only the retrieved passages (temp=0.2). Refuses to use outside knowledge."),
            ("7. Bare LLM",     "Same model, no context injected, higher temperature (0.9). Illustrates training-data-only responses."),
        ]
    ...
    with c_right:
        ...
        <p style="font-size:13px;color:#374151;font-family:'Inter',sans-serif;
            ...
            configurations (200, 500, 1000 tokens). Gemini auto-generates 8 factual
